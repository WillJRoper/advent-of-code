...
can be ignored; every other column has a reflected column within the pattern
and must match exactly: column 2 matches column 9, column 3 matches 8, 4
...
with open(fpath, "r") as f:
    linesss = f.read().strip().split("\n\n")

# Split the set of liness into individual sets of lines
liness = [lines.split("\n") for lines in linesss]

# Part 1

# Convert strings into arrays of 1s and zeros
arrs = []
for lines in liness:
...
def find_row_reflection(arr):
    """
    Test all rows and return the row index of reflection multiplied by 100
    if found.
    """

    # See if rows are equal
    for i in range(0, arr.shape[0] - 1):
        low_slice = slice(None, i + 1)
        high_slice = slice(i + 1, i + 1 + len(arr[low_slice]))
        high = np.flipud(arr[high_slice])
        low = arr[low_slice][-len(high) :]
        if np.all(low == high):
            break
    else:
        i = -1
...
def find_column_reflection(arr):
    """
    Test all columns and return the column index of reflection if found.
    """

    # See if columns are equal
    for i in range(0, arr.shape[1] - 1):
        low_slice = slice(None, i + 1)
        high_slice = slice(i + 1, i + 1 + arr[:, low_slice].shape[1])
        high = np.fliplr(arr[:, high_slice])
        low = arr[:, low_slice][:, -high.shape[1] :]
        if np.all(low == high):
            break
    else:
        i = -1
...
result = 0
for arr in arrs:
    result += find_row_reflection(arr)
    result += find_column_reflection(arr)
