...
def get_n_matches(scratch_card):
    """
    Get the number of wins on a card.

    Args:
        scratch_card (string)
            The string containing the scratch card numbers.

    Returns:
        int
            The number of matches.
    """
    # Split the scratch card
    winning, numbers = [lst.split(" ") for lst in scratch_card.split(":")[1].split("|")]

    # Clean up numbers and convert to sets
    winning = set([int(num) for num in winning if num != ""])
    numbers = set([int(num) for num in numbers if num != ""])

    # Find the number of matches
    n_matches = len(numbers.intersection(winning))
...
def recursive_cards(cards, n_cards, all_cards):
    """
    Recursively collect all cards generated by winning scratch cards.

    Args:
        cards (list)
            The current list of cards
        n_cards (dict)
            A dictionary containing the current number of each card.
        all_cards (list)
            A list contain the strings describing all scratch cards.

    Returns:
        n_cards (dict)
            The card dict after finding the extra won cards.
    """

    # Define a list to store new cards won in this recursion
    new_cards = []

    # Loop over the cards to process
    for card in cards:
        # Did we win?
        n_new = get_n_matches(card)

        # Get the card numbers
        card_id = card.split(":")[0]

        # Account for this card
        n_cards.setdefault(card_id, 0)  # only necessary on first loop
        n_cards[card_id] += 1

        # Get the index of this card
        card_ind = int(card_id.split(" ")[-1]) - 1

        # Get new cards
        for i in range(1, n_new + 1):
            new_cards.append(all_cards[card_ind + i])

    # Recurse if we have new cards
    if len(new_cards) > 0:
        print(len(new_cards))
        n_cards = recursive_cards(new_cards, n_cards, all_cards)

    return n_cards


# Setup the resulting dict
n_cards = {}

# GO!
recursive_cards(lines, n_cards, lines)

# Compute the result
result = sum(n_cards.values())
