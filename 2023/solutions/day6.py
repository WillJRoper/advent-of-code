...
dists = [int(t) for t in lines[1].split(":")[-1].strip().split(" ") if t != ""]


def quadratic_roots(a, b, c):
    """
    Compute the roots of a quandratic equation.
...
def get_n_wins(time, dist):
    """
    Count how many ways there are to win.
    """
    nwins = 0
...
# Find and multiply together the number of ways to win
result = 1
for t, d in zip(times, dists):
    result *= get_n_wins(t, d)
...
# Could brute force again but numbers are now large enough to use the
# quadratic formula
r1, r2 = quadratic_roots(a=1, b=-time, c=dist)
print("Part 2:", round(r1 - r2))
