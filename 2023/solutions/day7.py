...
class Hand:
    ...
    def get_type(self):
        # Set up a dictionary to count cards in
        card_counts = {k: 0 for k in set(self.hand)}
    ...
    def calc_score(self):
        self.score = self.rank * self.bid
        return self.score


# Parse the hands and get their type
hands = []
for line in lines:
    if len(line) == 0:
        continue
...
# Store each hand in a type split dicitonary
hand_types = {i: [] for i in range(1, 8)}
for hand in hands:
...
for hand_type in hand_types:
    for i in range(4, -1, -1):
        hand_types[hand_type] = list(
            sorted(hand_types[hand_type], key=lambda x: x.card_values[x.hand[i]])
        )

# Get the result by assigning and summing ranks
result = 0
rank = 1
for hand_type in hand_types:
    for hand in hand_types[hand_type]:
        hand.rank = rank
        result += hand.calc_score()
        rank += 1
...
class Hand:
    ...
    def get_type(self):
        if "J" in self.hand:
            # Define a list to store possible ranks
            poss_types = []

            for c in self.card_values:
                # Replace J with this card value
                hand = self.hand.replace("J", c)

                # Set up a dictionary to count cards in
                card_counts = {k: 0 for k in set(hand)}
                ...
                # Define ranks
                if len(card_counts) == 1:
                    poss_types.append(7)  # five of a kind
                elif len(card_counts) == 2:
                    if min(card_counts.values()) == 1:
                        poss_types.append(6)  # four of a kind
                    else:
                        poss_types.append(5)  # full house
                elif len(card_counts) == 3:
                    if max(card_counts.values()) == 3:
                        poss_types.append(4)  # three of a kind
                    else:
                        poss_types.append(3)  # two pair
                elif len(card_counts) == 4:
                    poss_types.append(2)  # one pair
                else:
                    poss_types.append(1)  # high card

            self.hand_type = max(poss_types)
